...
class EVA():
    def __init__(self, DATA, param):
        ...
        self.EV_P_dischar_max = DATA['EV_P_dischar_max']
        ...
        self.EVA_P = np.zeros((self.EVA_num, param.T*2))
        self.EVA_C = np.zeros((self.EVA_num, param.T*2))

        EVA_ub = np.zeros((self.EVA_num, param.T*2))
        EVA_lb = np.zeros((self.EVA_num, param.T*2))
        EVA_P_char_max = np.zeros((self.EVA_num, param.T*2))
        EVA_P_dischar_max = np.zeros((self.EVA_num, param.T*2))
        EVA_C_out = np.zeros((self.EVA_num, param.T*2))  # 电动汽车离开时带走的电量
        for i in range(self.EV_num):
            ...
            if self.EV_T_out[i]-self.EV_T_in[i] < delta_T:
                ...
                EVA_P_dischar_max[int(np.argwhere(self.EVA_BUS == self.EV_BUS[i])[0]), self.EV_T_in[i]:self.EV_T_out[i] + 1] += self.EV_P_dischar_max[i]
        ...
        self.EVA_P_dischar_max = EVA_P_dischar_max
    ...
    def EV_distribution(self, DICT):
        ...
        for i in range(self.EV_num):
            ...
            if self.EV_T_out[i] - self.EV_T_in[i] < delta_T:
                EV_lb[i, self.EV_T_out[i]] = (self.EV_T_out[i] - self.EV_T_in[i]) * EV_P_char_max[i] * DICT['Param'].SB
                for t in range(1, self.EV_T_out[i] - self.EV_T_in[i] + 1):
                    EV_lb[i, self.EV_T_out[i] - t] = EV_lb[i, self.EV_T_out[i] - t + 1] - EV_P_char_max[i]
            else:
                EV_lb[i,self.EV_T_out[i]] = self.EV_C_max[i]*(self.EV_SOC_out[i]-self.EV_SOC_in[i]) * DICT['Param'].SB
                for t in range(1, self.EV_T_out[i] - self.EV_T_in[i] + 1):
                    EVA_P = self.EVA_P[int(np.argwhere(self.EVA_BUS == self.EV_BUS[i])[0]), self.EV_T_out[i] - t + 1] * DICT['Param'].SB
                    EV_lb[i, self.EV_T_out[i] - t] = EV_lb[i, self.EV_T_out[i] - t + 1] - (min(EV_P_char_max[i], EVA_P) if EVA_P>=0 else max(EV_P_dischar_max[i], EVA_P))
        ...
        for i in range(self.EVA_num):
            for t in range(T):
                ...
                if len(num) != 0:
                    try:
                        EV_ = model.addVars(len(num), vtype=GRB.CONTINUOUS, lb=EV_P_dischar_max[num], ub=EV_P_char_max[num], name='EV_P')
                        lambda_ = model.addVar(lb=-GRB.INFINITY,vtype=GRB.CONTINUOUS, name='lambda')

                        # model.addConstrs((EV_[j] >= EV_lb[num[j], t] - np.sum(self.EV_P[num[j], :t]) for j in range(len(num))), name='EV_lb')
                        model.addConstr((quicksum(EV_[k] * SOC[num[k]] for k in range(len(EV_))) == lambda_ * len(EV_)), name='A')
                        model.addConstr((quicksum(EV_[k] for k in range(len(EV_))) == self.EVA_P[i, t] * DICT['Param'].SB), name='B')
                        ...
                        EV_P = single_var(EV_, len(EV_))
                        for k in range(len(EV_)):
                            SOC[num[k]] += EV_P[k] / self.EV_C_max[num[k]] / DICT['Param'].SB
