...
class ID():
    ...
    def SP(self, DICT, L):
        ...
        EVA_lb = DICT['EVA'].EVA_lb[:,:T]
        EVA_ub = DICT['EVA'].EVA_ub[:,:T]
        EVA_P_dischar_max = DICT['EVA'].EVA_P_dischar_max[:,:T]
        EVA_P_char_max = DICT['EVA'].EVA_P_char_max[:,:T]
        EVA_C_out = DICT['EVA'].EVA_C_out[:,:T]
        ...
        for t in range(T):
            ...
            for i in range(EVA_num):
                model.addConstr((EVA_C[i, t] == quicksum(EVA_[i, k] - EVA_C_out[i, k] for k in range(t))), name='EVA_C')
                model.addConstr((EVA_C[i, t] <= EVA_ub[i, t]), name='EVA_C_max')  # 电动汽车当天累计充电电量减去汽车离开时的所冲电量落在充电功率曲线中
                model.addConstr((EVA_C[i, t] >= EVA_lb[i, t]), name='EVA_C_min')
        ...
        model.setObjective(quicksum((EVA_C[i, t] - L['EVA_C'][i, t])**2 for i in range(EVA_num) for t in range(T)), GRB.MINIMIZE)
        ...
        DICT['EVA'].EVA_P = show.double_var(EVA_, EVA_num, T)
        DICT['EVA'].EVA_C = show.double_var(EVA_C, EVA_num, T)
