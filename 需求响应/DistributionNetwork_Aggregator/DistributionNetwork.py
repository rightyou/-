...
class DISTR():
    ...
    def AdjustableBoundaryAggregation(self, DICT):
        ...
        Is_UP = model.addVars(int(Branch_num), T, vtype=GRB.CONTINUOUS, lb=0, ub=500, name='Is')
        ...
        P_UP_branch = model.addVars(int(Branch_num), T, vtype=GRB.CONTINUOUS, lb=0, ub=GRB.INFINITY, name='P_branch')
        Q_UP_branch = model.addVars(int(Branch_num), T, vtype=GRB.CONTINUOUS, lb=0, ub=GRB.INFINITY, name='Q_branch')
        ...
        for t in range(T):
            ...
            for i in range(EVA_num):
                ...
                model.addConstr((C_EVA_UP[i, t] >= EVA_lb[i, t]), name='EVA_C_min')
        ...
        for t_ in range(self.T_Adjust0):
            model.addConstrs((P_UP_input[i, t_] - P_LO_input[i, t_] == 0 for i in range(BUS_num)), name='control')

        for t_ in range(self.T_Adjust0, self.T_Adjust1):
            model.addConstrs((P_UP_input[i, t_] >= P_LO_input[i, t_] for i in range(BUS_num)), name='control')

        # 目标设置
        model.setObjective(quicksum(P_UP_input[0, t_] - P_LO_input[0, t_]*100 for t_ in range(self.T_Adjust0, self.T_Adjust1)), GRB.MAXIMIZE)
        ...
        try:
            ...
            model.optimize()
            P_UP_input_ = double_var(P_UP_input, EVA_num, T)
            P_LO_input_ = double_var(P_LO_input, EVA_num, T)
            self.P_UP = P_UP_input_[0, :self.T_Adjust1]
        ...
        # # plt.plot(t, P_UP_input_[0], drawstyle='steps', label='可调上界')
        ...
        # #     df = pd.DataFrame(np.stack((P_LO_input_[0], P_UP_input_[0], disordered_charging.sum(0)[:96] + ED.sum(0)[:96])))
    ...
    def DistributionNetwork_DA(self, DICT, DistributionNetwork_P):
        ...
        model = Model('DA')
        Us_UP = model.addVars(int(BUS_num), T, vtype=GRB.CONTINUOUS, lb=9.5 ** 2, ub=10.5 ** 2, name='Us')
        Is_UP = model.addVars(int(Branch_num), T, vtype=GRB.CONTINUOUS, lb=0, ub=GRB.INFINITY, name='Is')
        P_UP_input = model.addVars(int(BUS_num), T, vtype=GRB.CONTINUOUS, name='P_input')
        Q_UP_input = model.addVars(int(BUS_num), T, vtype=GRB.CONTINUOUS, name='Q_input')
        P_UP_branch = model.addVars(int(Branch_num), T, vtype=GRB.CONTINUOUS, lb=0, ub=GRB.INFINITY, name='P_branch')
        Q_UP_branch = model.addVars(int(Branch_num), T, vtype=GRB.CONTINUOUS, lb=0, ub=GRB.INFINITY, name='Q_branch')
        ...
        Us_LO = model.addVars(int(BUS_num), T, vtype=GRB.CONTINUOUS, lb=9.5 ** 2, ub=10.5 ** 2, name='Us')
        Is_LO = model.addVars(int(Branch_num), T, vtype=GRB.CONTINUOUS, lb=0, ub=GRB.INFINITY, name='Is')
        P_LO_input = model.addVars(int(BUS_num), T, vtype=GRB.CONTINUOUS, name='P_input')
        Q_LO_input = model.addVars(int(BUS_num), T, vtype=GRB.CONTINUOUS, name='Q_input')
        P_LO_branch = model.addVars(int(Branch_num), T, vtype=GRB.CONTINUOUS, lb=0, ub=GRB.INFINITY, name='P_branch')
        Q_LO_branch = model.addVars(int(Branch_num), T, vtype=GRB.CONTINUOUS, lb=0, ub=GRB.INFINITY, name='Q_branch')
        ...
        for t in range(T):
            ...
            for i in range(Branch_num):
                ...
                model.addConstr(4 * P_UP_branch[i, t] ** 2 + 4 * Q_UP_branch[i, t] ** 2 + (Is_UP[i, t] - Us_UP[head, t]) ** 2 <= (
                            Is_UP[i, t] + Us_UP[head, t]) ** 2)
                model.addConstr(4 * P_LO_branch[i, t] ** 2 + 4 * Q_LO_branch[i, t] ** 2 + (Is_LO[i, t] - Us_LO[head, t]) ** 2 <= (
                            Is_LO[i, t] + Us_LO[head, t]) ** 2)

        # 保证调度时段前运行方式保持一致
        for t_ in range(self.T_Adjust1):
            model.addConstrs((P_UP_input[i, t_] - P_LO_input[i, t_] == 0 for i in range(BUS_num)), name='control')
            model.addConstrs((quicksum(P_UP_input[i, t_] for i in range(BUS_num)) == DistributionNetwork_P[t_]), name='control')
            model.addConstrs((quicksum(P_LO_input[i, t_] for i in range(BUS_num)) == DistributionNetwork_P[t_]), name='control')
