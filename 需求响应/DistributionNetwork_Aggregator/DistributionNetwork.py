...
class DISTR():
    ...
    def AdjustableBoundaryAggregation(self, DICT):
        ...
        P_UP_input = model.addVars(int(BUS_num), T, vtype=GRB.CONTINUOUS, name='P_UP_input')
        ...
        P_LO_input = model.addVars(int(BUS_num), T, vtype=GRB.CONTINUOUS, name='P_UP_input')
        ...
        for t in range(T):
            ...
            model.addConstrs((P_UP_input[i, t] ==
                              ...
                              for i in range(1, BUS_num)), name='P_UP_input')  # 节点0与上层电网相连接
        ...
        # model.setParam("MIPGap", 0)
        # model.optimize()
        # P_input_ = show.double_var(P_UP_input, EVA_num, T)
    ...
    def DistributionNetwork_DA(self, DICT, DistributionNetwork_P):
        ...
        try:
            ...
            model.setParam("MIPGap", 0)
            model.optimize()
            self.DA_P = double_var(P_input, EVA_num, T)[0]
        except:
            model.computeIIS()
            model.write('model.ilp')
            pass
