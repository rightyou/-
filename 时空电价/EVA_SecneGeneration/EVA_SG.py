...
def montecarlo(path, Price):
    param = Param()
    simulate_num = 1

    DATA = read_Road_data(param, '{}/Road.xlsx'.format(path))
    road = Road(DATA, param)
    DATA = read_ChargingStation_data(param, '{}/Charging station.xlsx'.format(path), Price)
    cs = CS(DATA)  # 充电站信息
    ...
    for k in range(simulate_num):
        ...
        # with pd.ExcelWriter('road2.xlsx') as writer:
        #     df = pd.DataFrame(DICT['Road'].Road_flow[:112] + DICT['Road'].Road_flow[112:])
    ...
    for i in range(monte_carlo['EVA_num']):
        for j in range(96 * 2):
            ...
            disordered_charging[i, j] = monte_carlo['EVA_ub'][i, j] - monte_carlo['EVA_ub'][i, j - 1] + monte_carlo['EVA_C_out'][i, j]
    load = pd.read_excel('data/DistributionNetwork/EDBase.xlsx', header=0, index_col=0).values[:, 1:97]  # 节点负荷信息
...
def draw(disordered_charging, load):
    plt.figure()
    plt.plot((disordered_charging[:, :96] * 2 * 1000 + load).sum(0))
    plt.show()

    with pd.ExcelWriter('disorder2.xlsx') as writer:
        df = pd.DataFrame((disordered_charging[:, :96] * 2 * 1000 + load).T)
        df.to_excel(writer, sheet_name='Sheet1', index=None)