...
class Car():
    ...
    def charge_route_generate(self, DICT, i, t, weight):
        '''
        w1 充电电价权重
        w2 路线成本权重（包含耗电和时间成本，对于一个只考虑电价最低，不考虑时间成本的用户，有w1=w1=0.5,因此w2的取值范围为[0.5，1]）
        '''
        ...
        for j in range(DICT['CS'].CS_num):
            self.Car_destination[i] = DICT['CS'].CS_area[j]
            price = DICT['CS'].CS_Price[j, t // 120]
            route_, r_cost_ = Taxi.destination_route_generate(self, DICT, i, t, weight)
            cost_ = w1*price*power + w2*r_cost_
            if cost_ < cost:
                cost = cost_
                route = route_
                m = j
        self.Car_destination[i] = DICT['CS'].CS_area[m]
        return route
...
class Taxi(Car):
        ...
        for i in range(self.Car_num):
            ...
            if t//t_ > self.Car_T_end[i] and self.Car_area_T[i] == self.Car_area_start[i]:
                if not self.Car_charge_flag[i]:
                    ...
                    data['EV_P_dischar_max'] = np.append(data['EV_P_dischar_max'], self.Car_P_discharge[i] * t_)
                    data['EV_P_char_max'] = np.append(data['EV_P_char_max'], self.Car_P_charge[i] * t_)
            ...
            # 目的地选取
            if self.Car_destination[i] == 0:
                Taxi.destination_generate(self, DICT, i, int(t//t_))
                self.Car_route[i],_ = Taxi.destination_route_generate(self, DICT, i, t, weight)
                self.Car_road_T[i] = self.Car_route[i][0]
                DICT['Road'].Road_flow[self.Car_road_T[i], t] += 1
            ...
            if self.Car_next[i] > 1:
                DICT['Road'].Road_flow[self.Car_road_T[i], t] -= 1
                remainder = (self.Car_next[i]-1) * DICT['Road'].Road_length[j] / speed
                while remainder > 0:
                    ...
                    beta = DICT['Road'].a_b_n[DICT['Road'].Road_grade[j] - 1][0] + \
                           ...
                           DICT['Road'].a_b_n[DICT['Road'].Road_grade[j] - 1][2]
                    speed = self.Car_v[i] / (1 + (DICT['Road'].Road_flow[j, t] / DICT['Road'].Road_capacity[j]) ** beta)
                    remainder -= DICT['Road'].Road_length[j] / speed
    ...
    def destination_generate(self, DICT, i, t_):
        while True:
            destination_probability = DICT['area'].destination_probability[t_]
            destination_flag = random.randint(1, sum(destination_probability))
            k = 0
            for m in range(len(destination_probability)):
                k += destination_probability[m]
                if k >= destination_flag:
                    self.Car_destination[i] = m+1
                    break
